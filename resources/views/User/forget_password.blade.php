...
<div class="login-box">
  ...
  <div class="card">
    ...
    <div class="card-body login-card-body">
      <p class="login-box-msg">Enter your email to generate a password reset link </p>
      @if (session('error'))
      <div class="text-danger mx-2 p-2 ">
          *{{ session('error') }}

      </div>
      @endif
      @if (session('success'))
          <div class="text-success mx-2 p-2 ">
              {{ session('success') }}
          </div>
      @endif

      <form action="{{route('forget-sendmail')}}" method="post">
        @csrf
